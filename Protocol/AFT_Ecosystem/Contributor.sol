 ...
 *Submitted for verification at polygonscan.com on 2022-08-03
 Polygon Mainnet: 0x8f81F9e445eCEd350a74ce93d4dA30d0759F500C
*/
...
contract NFMContributor {
     ...
     */
    //------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    function _addContributor(string calldata ctyp, address ContToAdd)
        public
        returns (bool)
    {
        address user;
        ...
        if (bal == true) {
            if (_contributorCheck[user] == false) {
                ...
                contributorAll.push(Contributor(user, ctyp, block.timestamp));
     ...
     */
    //------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    function _returnProjects(uint256 allAccComp)
        public
        view
        returns (Project[] memory output)
    {
        if (allAccComp == 1) {
            return projectsAll;
     ...
     */
    //------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    function _returnProjectsContributor(address ContributorsAll)
        public
        view
        returns (Project[] memory output)
    {
     ...
     */
    //------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    function _returnProjectInfo(uint256 prinfo)
        public
        view
        returns (Project memory output)
    {
     ...
     */
    //------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    function _returnProjectNotify(uint256 pr)
        public
        view
     ...
     */
    //------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    function _returnAllContributors()
        public
        view
        returns (Contributor[] memory output)
    {
    ...
    function _addProject(
        string calldata ptyp,
        string calldata pname,
        string calldata pdesc,
        ...
        if (IERC20(_Controller._getNFM()).balanceOf(msg.sender) > 10 * 10**18) {
            if (_contributorCheck[msg.sender] == true) {
                ...
                ContributorProjects[msg.sender].push(
                    Project(
                        projectAllCounter,
                        ptyp,
                        pname,
                ...
                _projectNotification[
                    projectAllCounter
                ] = "Your project request has been listed. If your project is accepted, you get access to additional functions.";
                projectAllCounter++;
            }
        }
        return true;
    ...
    function _setProjectCompleted(uint256 ProjectID) public returns (bool) {
        if (msg.sender == _projectInfo[ProjectID].contributor) {
            ...
            for (
                uint256 i = 0;
                i < ContributorProjects[msg.sender].length;
                i++
            ) {
                if (ContributorProjects[msg.sender][i].projectID == ProjectID) {
                    ContributorProjects[msg.sender][i].enddate = block
                        .timestamp;
            ...
            _projectNotification[
                ProjectID
            ] = "Your project will now be checked! If accepted, you will be listed for the reward. Otherwise, it reverts to development.";
        }
        return true;
    }
    ...
    function _approveReward(
        ...
        if (stat == true) {
            ...
            INfmTreasury(_Controller._getTreasury())._getWithdraw(
                _Controller._getNFM(),
                address(this),
                Amount,
                false
            ...
            _projectNotification[
                ProjectID
            ] = "Congratulations, your project has been accepted. The reward will be paid out on the next distribution date.";
        } else {
            _projectNotification[
                ProjectID
            ] = "Your project does not sufficiently fulfill its purpose. More work needed.";
     ...
     */
    //------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    function approveOrDenyProject(uint256 ProjectID, bool Stat)
        public
        onlyOwner
        returns (bool)
    {
        ...
        if (Stat == true) {
            _projectNotification[
                ProjectID
            ] = "Congratulations! Your project does match our requirements.";
            projectsAccepted.push(_projectInfo[ProjectID]);
            projectAcceptedCounter++;
        } else {
            _projectNotification[
                ProjectID
            ] = "Your project does not match our requirements.";
        }
        return true;
    }
    ...
    function payRewardsToContributors() public onlyOwner returns (bool) {
        ...
        if (AmountsOfPayments > 0) {
            ...
            for (uint256 i = 0; i < RewardAddressArray.length; i++) {
                if (
                    IERC20(_Controller._getNFM()).transfer(
                        RewardAddressArray[i],
                        RewardAmountArray[i]
                    ) == true
                ) {
                    zaehler++;
